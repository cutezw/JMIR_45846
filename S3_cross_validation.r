...
model_names <- c("combination_1","combination_2","combination_3",
"combination_4","stl_arima","snaive","sarima","nnetar")
...
pdf("cv_mae.pdf")
horizon_acc %>%
  ggplot(aes(x = h, y = MAE, colour = model, shape = model)) +
  geom_point(size = 2.5) +
  ...
  theme(
    ...
    legend.key.size = unit(15, "pt"), legend.text = element_text(size = 8)
  ) +
...
pdf("cv_rmse.pdf")
horizon_acc %>%
  ggplot(aes(x = h, y = RMSE, colour = model, shape = model)) +
  geom_point(size = 2.5) +
  xlab("Forecast horizon (day)") +
  theme(
    legend.position = "bottom",
...
# Plot boxplot
box_data <- data.frame(
  month = month_acc$m, RMSE = month_acc$RMSE, MAE = month_acc$MAE
)

pdf("month_rmse_boxplot.pdf")
boxplot(box_data$RMSE ~ box_data$m,
  boxwex = 0.8,
  col = rgb(0.2, 0.3, 0.8, 0.5),
  ylab = "RMSE", xlab = "Month"
)
dev.off()

pdf("month_mae_boxplot.pdf")
boxplot(box_data$MAE ~ box_data$m,
  boxwex = 0.8,
  col = rgb(0.2, 0.3, 0.8, 0.5),
  ylab = "MAE", xlab = "Month"
)
...
for(m in model_names){
  if(exists("acc_15_days")){
    acc_15_days <- acc_15_days %>% add_row(acc_of_consequent_days(m, 15))
  }else {
