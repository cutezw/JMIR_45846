...
policy_interrupt <- vis_data %>% filter(time == as.Date("2022-12-19"))

# Plot time plot of the dataset
pdf("time_plot.pdf", width = 7, height = 5)
ggplot(vis_data, aes(x=time, y=visits)) + geom_line() + labs(y = "Hourly visits", x="Year") + 
geom_vline(xintercept = as.numeric(vis_data$time[policy_interrupt$...1]), linetype="dashed", color = "red", linewidth=0.5)
dev.off()
...
pdf("acf_dif.pdf")
vis_data %>% gg_tsdisplay(difference(visits, 24), lag_max = 48, plot_type = "partial")
dev.off()

# Plot the STL decomposition of the time series
pdf("stl.pdf")
vis_decomp <- vis_data %>%
    model(STL(visits ~ season(period = 24) +
        season(period = 24 * 7) +
