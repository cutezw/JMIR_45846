...
vis_data <- readr::read_csv("core/open_hourly_visits.csv") %>% as_tsibble(index = time)
...
fig_map <- tibble(m_name = c("snaive","sarima","nnetar","stl_arima","combination_1","combination_2","combination_3","combination_4"),
  t_name = c("SNaïve","SARIMA","NNAR","STLF","Hybrid SARIMA-STLF","Hybrid NNAR-STLF","Hybrid SARIMA-NNAR","Hybrid SARIMA-NNAR-STLF"))

for(i in seq(nrow(fig_map))){
  model_fc <- com_fc %>% filter(.model == fig_map[i,]$m_name) 
  vis_future$fc_visits <- model_fc$.mean

  pdf(paste("results_",fig_map[i,]$t_name,".pdf", sep=""), width = 7, height = 5)
  ggplot(vis_future, aes(x = time)) +
  geom_line(aes(y = visits, color = "Observation")) +
  geom_line(aes(y = fc_visits, color = paste(fig_map[i,]$t_name, "Prediction", sep=" "))) +
  scale_color_manual('', values=c("#1cdf3cc8","#1d55e1c7"), breaks=c(paste(fig_map[i,]$t_name, "Prediction", sep=" "), "Observation")) +
  labs(x = 'Hour', y = 'Visits') +
  theme(legend.position=c(0,1),legend.justification = c("left", "top"),legend.key = element_blank(), legend.title = element_blank(),legend.background = element_rect(fill = "white", colour = "gray", linewidth=0.1),
...
# RMSE, MAE
acc_data <- com_acc %>% mutate(model = recode(.model,
  snaive = "SNaïve", sarima = "SARIMA", stl_arima = "STLF",
  nnetar = "NNAR",
  combination_1 = "Hybrid SARIMA-STLF", combination_2 = "Hybrid NNAR-STLF",
  combination_3 = "Hybrid SARIMA-NNAR", combination_4 = "Hybrid SARIMA-NNAR-STLF"
...
tiff(
  filename = "acc_bar.tiff", width = 20, height = 15,
  units = "cm", res = 300
)
acc_data %>%
  pivot_longer(cols = RMSE:MAE, names_to = "measure", values_to = "value") %>%
  ...
  scale_pattern_type_discrete(choices = c("gray50", "hs_bdiagonal", "hs_horizontal", "hs_diagcross", "hs_vertical", "gray100", "hs_fdiagonal", "gray0")) +
  facet_wrap(~`measure`) +
  theme(
    axis.text.x = element_blank(), axis.title.x = element_blank(), axis.ticks.x = element_blank(),
    axis.title.y = element_blank()
  ) +
  geom_text(aes(label = round(value, 2)), vjust = 0, color = "black", size = 2.8) +
  ...
  theme(
    ...
    legend.key.size = unit(20, "pt"), legend.text = element_text(size = 9)
  ) +
